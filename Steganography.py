...
class Stegno:
    ...
    def frame2_decode(self, d_f2):
        d_f3 = Frame(root)
        myfile = tkinter.filedialog.askopenfilename(filetypes=[('Image Files', '*.png;*.jpeg;*.jpg;*.bmp;*.gif;*.ppm;*.pgm;*.pbm;*.tiff'),
                                                               ('All Files', '*.*')])
        if not myfile:
    ...
    def enc_fun(self, text_area, myimg):
        ...
        if (len(data) == 0):
            ...
            image_file_types = [('PNG', '*.png'), ('All Files', '*.*')]
            file_name = tkinter.filedialog.asksaveasfilename(initialfile=temp, filetypes=image_file_types,
                                                             defaultextension=".png")
            newimg.save(file_name)
            self.d_image_size = my_file.tell()
            self.d_image_w, self.d_image_h = newimg.size
            messagebox.showinfo("Success", "Encoding Successful\nFile is saved as {} in the same directory".format(file_name))
